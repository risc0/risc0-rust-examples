...
static void println(char *fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
  int written = sprintf(output_buf + output_cursor, fmt, ap);
  output_cursor += written;
  va_end(ap);
  written = sprintf(output_buf + output_cursor, "\n");
  output_cursor += written;
}
...
void codegen_mem_buf(Obj *prog, char *bin_buf) {
  output_buf = bin_buf;
  File **files = get_input_files();
  for (int i = 0; files[i]; i++)
    println("  .file %d \"%s\"", files[i]->file_no, files[i]->name);
