...
Token *tokenize_input_string(char* p) {
  current_file = malloc(sizeof(File));
  current_file->contents = p;
  char *name = malloc(2);
  name[0] = 'i';
  name[1] = '\0';
  current_file->name = name;
  current_file->display_name = name;
  current_file->file_no = 0;
  input_files = malloc(sizeof(File*) * 2);
  input_files[0] = current_file;
  input_files[1] = NULL;
  
  return tokenize(current_file);
}
