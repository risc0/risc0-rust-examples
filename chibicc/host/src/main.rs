...
use methods::{COMPILE_ID, COMPILE_PATH};
...
fn main() {
    env_logger::init();
    // Make the prover.
    let method_code = std::fs::read(COMPILE_PATH)
        .expect("Method code should be present at the specified path; did you use the correct *_PATH constant?");
    let mut prover = Prover::new(&method_code, COMPILE_ID).expect(
    ...
    receipt.verify(COMPILE_ID).expect(
